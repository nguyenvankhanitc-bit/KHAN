# -*- coding: utf-8 -*-

from . import phan_he_currency
from . import phan_he_org
from . import phan_he_service_type
from . import phan_he_module_access
from . import lug_menu_permission
from . import menu_access
from . import lug_menu_access
from . import phan_he_work_shift
from . import phan_he_shift_roster
from . import monthly_matrix_schedule
from . import monthly_roster_send_mail
from . import linkq_today_shift
from . import monthly_shift_sheet
from . import roster_lock
from . import store_notification
from . import guide_document
from . import shift_guide_document
from . import shift_summary_report
from . import store_dashboard
from . import shift_code
from . import shift_schedule_north
from . import shift_schedule_dtt
from . import shift_management
from . import phan_he_store
from . import phan_he_provider
from . import phan_he_access_mixin
from . import phan_he_service
from . import phan_he_payment
from . import phan_he_payment_file
from . import phan_he_invoice
from . import phan_he_document
from . import phan_he_dashboard
from . import res_users
from . import hr_employee
from . import hr_store
from . import linkq_employee
from . import store_permission
from . import ir_ui_menu
